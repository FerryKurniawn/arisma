...
const DaftarDisposisi = () => {
  ...
  const fetchData = () => {
    fetch("http://localhost:2000/api/surat-masuk")  // Ganti dengan endpoint yang sesuai jika perlu
      .then((res) => res.json())
      .then((data) => {
        setDataSurat(data);
  ...
  const handleSearch = () => {
    const filtered = dataSurat.filter((surat) => {
      ...
      return (
        ...
        (surat.isiDisposisi && surat.isiDisposisi.toLowerCase().includes(search))
      );
    });
        ...
        <div className="p-8">
          ...
          <div className="flex justify-end mt-6">
            <button
              ...
              onClick={() => navigate("/kepsek/tambah-disposisi")}
            >
              Tambah
            </button>
          </div>
